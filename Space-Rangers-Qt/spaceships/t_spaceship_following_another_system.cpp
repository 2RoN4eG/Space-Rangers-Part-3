...
void t_spaceship_following_another_system::update(const t_delta_timestamp delta) {
    ...
    const t_2d_vector_entity directed_distance = make_2d_vector(follower_position, target_position);

    const t_2d_vector_length distance_length = t_2d_vector_length_system(directed_distance);

    if (distance_length <= _within_range) {
        return;
    }

    const float steps = distance_length / _stalker.get_speed();

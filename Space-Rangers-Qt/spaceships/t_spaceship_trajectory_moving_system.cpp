...
t_spaceship_trajectory_moving_system::t_spaceship_trajectory_moving_system(t_scene_components& scene_components, const t_spaceship_id_entity spaceship_id)
    : t_spaceship_trajectory_moving_system { scene_components.spaceship(spaceship_id) }
{
}
...
void t_spaceship_trajectory_moving_system::update([[maybe_unused]] t_delta delta) {
    ...
    const t_scalar_length_entity distance_length = t_2d_vector_length_system(heading);
    ...
    const t_scalar_length_entity heding_length = t_2d_vector_length_system(heading);

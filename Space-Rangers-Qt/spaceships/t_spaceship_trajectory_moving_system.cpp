...
void t_spaceship_trajectory_moving_system::update([[maybe_unused]] t_delta delta) {
    ...
    t_2d_position& spaceship_position = _spaceship.position();

    const t_2d_position& target_position = trajectory.front();
    
    const t_2d_vector heading = t_2d_system_make_vector(spaceship_position, target_position);
    
    const t_2d_vector_length distance_length = t_2d_vector_length_system(heading);

    const float frames_to_destination = distance_length / _spaceship.get_linear_speed();

    const t_step_per_frame step_per_frame = t_2d_vector_division_system(heading, frames_to_destination);

...
t_angle_entity t_convert_from_vector_to_radians_system(const t_2d_vector& vector) {
...
t_2d_position t_2d_make_position_by_x_axis_system(const t_angle_entity degrees, const t_radius_entity radius) {
    const t_angle_entity radians = t_convert_angle_from_degrees_to_radians_system(degrees);

    return { radius * std::cos(radians), radius * std::sin(radians) };
}

t_2d_position t_2d_make_position_by_y_axis_system(const t_angle_entity degrees, const t_radius_entity radius) {
    return t_2d_make_position_by_x_axis_system(degrees + t_angle_entity { 90. }, radius);
}

t_2d_position t_2d_make_position_by_x_axis_system(const t_angle_entity degrees, const t_radius_entity radius, const t_2d_position& around) {
    const t_2d_position&& position = t_2d_make_position_by_x_axis_system(degrees, radius);

    return t_2d_vector_addition_system(around, position);
}

t_2d_position t_2d_make_position_by_y_axis_system(const t_angle_entity degrees, const t_radius_entity radius, const t_2d_position& around) {
    const t_2d_position&& position = t_2d_make_position_by_y_axis_system(degrees, radius);
